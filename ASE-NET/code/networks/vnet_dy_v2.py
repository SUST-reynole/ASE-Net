...
class DYBAC_3D(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size,padding,stride=1):
    ...
    def __init__(self, n_stages, n_filters_in, n_filters_out, normalization='none'):
        ...
        for i in range(n_stages):
            ...
            if i ==1:
                ...
                ops.append(DYBAC_3D(input_channel, n_filters_out, 3, padding=1))
            if normalization == 'batchnorm':
                ops.append(nn.BatchNorm3d(n_filters_out))
            elif normalization == 'groupnorm':
                ops.append(nn.GroupNorm(num_groups=16, num_channels=n_filters_out))
        ...
        self.conv = nn.Sequential(*ops)
        # self.sp = sSE_Module_3D(n_filters_out)
    def forward(self, x):
        x = self.conv(x)
...
class ResidualConvBlock(nn.Module):
    def __init__(self, n_stages, n_filters_in, n_filters_out, normalization='none'):
        ...
        for i in range(n_stages):
            ...
            ops.append(Dynamic_conv3d(input_channel, n_filters_out, 3, padding=1))
            if normalization == 'batchnorm':
                ops.append(nn.BatchNorm3d(n_filters_out))
            elif normalization == 'groupnorm':
                ops.append(nn.GroupNorm(num_groups=16, num_channels=n_filters_out))
...
class VNet_dy(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        super(VNet_dy, self).__init__()
        self.has_dropout = has_dropout

        self.block_one = ConvBlock1(1, n_channels, n_filters, normalization=normalization)
...
if __name__ == '__main__':
    ...
    model = VNet_dy(n_channels=1, n_classes=2-1,
    ...
    print(flops, params)
    torch.save(model.state_dict(), 'V-Net_dy.pth')
